...
public class ScoreTrap extends SequentialCommandGroup {
    ...
    public ScoreTrap(Deployment sys_deployment, Cartridge sys_cartridge, Climber sys_climber) {
        addCommands(
                Commands.runOnce(() -> sys_climber.setPosition(Constants.kClimber.LOW, Constants.kClimber.KFAST_SLOT),
                        sys_climber),
                Commands.waitUntil(
                        () -> Math.abs(sys_climber.getPosition()) >= Math.abs(Constants.kClimber.TRAP_TRIGGER_POS)),
                Commands.runOnce(() -> sys_deployment.setPosition(kDeployment.setpoints.trap_pos), sys_cartridge),
                ...
                Commands.runOnce(() -> sys_deployment.setPosition(kDeployment.setpoints.home), sys_deployment));
    }
}