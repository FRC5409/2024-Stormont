...
public class AlignToPose extends Command {
	...
	public AlignToPose(Supplier<Pose2d> targetPoseSupplier, Drivetrain sys_Drivetrain, boolean doSlowMode,
		...
		if (doSlowMode) {
			m_xController = new PIDController(kAutoAlign.kPIDDriveSlow.T_CONTROLLER_P,
					kAutoAlign.kPIDDriveSlow.T_CONTROLLER_I, kAutoAlign.kPIDDriveSlow.T_CONTROLLER_D); // TODO

			m_xController.setTolerance(kAutoAlign.kPIDDriveSlow.T_CONTROLLER_TOLERANCE);

			m_yController = new PIDController(kAutoAlign.kPIDDriveSlow.T_CONTROLLER_P,
					kAutoAlign.kPIDDriveSlow.T_CONTROLLER_I, kAutoAlign.kPIDDriveSlow.T_CONTROLLER_D);
			...
			m_xController = new PIDController(kAutoAlign.kPIDDrive.T_CONTROLLER_P, kAutoAlign.kPIDDrive.T_CONTROLLER_I,
					kAutoAlign.kPIDDrive.T_CONTROLLER_D); // TODO

			m_xController.setTolerance(kAutoAlign.kPIDDrive.T_CONTROLLER_TOLERANCE);

			m_yController = new PIDController(kAutoAlign.kPIDDrive.T_CONTROLLER_P, kAutoAlign.kPIDDrive.T_CONTROLLER_I,
	...
	public void moveToPose(Pose2d targetPose) {
		...
		double xControllerOutput = applyTolerance(
				applyFeatForward(m_xController.calculate(currentPose.getX()), controllerTolerance), currentPose.getX(),
				targetPose.getX(), controllerTolerance);
		double yControllerOutput = applyTolerance(
				applyFeatForward(m_yController.calculate(currentPose.getY()), controllerTolerance), currentPose.getY(),
				targetPose.getY(), controllerTolerance);
		...
		FieldCentric driveCommand = sys_drivetrain.teleopDrive.withVelocityX(xControllerOutput)
				.withVelocityY(yControllerOutput).withRotationalRate(rControllerOutput);
	...
	private boolean isAligned(Pose2d targetPose) {
		...
		if (Math.abs(currentPose.getX() - targetPose.getX()) >= reachedPoseTolerance
				|| Math.abs(currentPose.getY() - targetPose.getY()) >= reachedPoseTolerance) {
